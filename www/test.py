...
async def create_pool(loop, **kw):
    ...
    #此处为全局变量,相当于在函数外定义了一个变量,但是有一点区别是,函数外声明的变量一旦import导入模块,就声明了,在函数内部通过global声明全局变量,函数不执行,就不会创建该全局变量(这里是个人理解,总之一点global声明的变量是全局变量,如果在函数内部声明的,该函数不执行,在函数外部访问时,会报错,因为函数不执行,该全局变量就未被声明)
    global __pool
...
async def test():
    ...
    await create_pool(loop=loop,host='127.0.0.1', port=3306,user='root', password='123',db='rpt')
    #没有设置默认值的一个都不能少
    u = User(name='Test', email='test@example.com', passwd='1234567890', image='about:blank',id="1567")
    await u.save()
